...
router.get(
  ...
  auth,
  supplierController.supplier_profile,
);
router.put(
  "/profile/personalDetails",
  auth,
...
router.get(
  ...
  auth,
  supplierController.get_bank_account_details,
);
router.post(
  "/stores/bankAccountDetails",
  auth,
  supplierController.supplier_bank_account_details,
);
router.put(
  "/stores/bankAccountDetails",
  auth,
  supplierController.update_bank_account_details,
);
...
router.post(
  ...
  auth,
  supplierController.add_gst_details,
);
...
router.post(
  ...
  supplierController.add_products,
);
router.get("/stores/products", auth, supplierController.get_products);
router.get(
  "/stores/products/bulk-price-reminders",
  auth,
...
router.put(
  ...
  auth,
  upload.array("images", 10),
  supplierController.update_product,
);

router.delete(
  "/stores/products/:product_id",
  auth,
  supplierController.delete_product,
);
router.post(
  "/stores/products/:product_id/variants",
  auth,
  upload.array("images", 10),
  supplierController.add_product_variants,
);
router.post(
  "/stores/variants/:variant_id/images",
  upload.array("images", 10),
  auth,
  supplierController.add_product_images,
);

//warehouse
router.post("/stores/warehouses", auth, supplierController.create_warehouse);
router.put(
  "/stores/warehouses/:warehouse_id",
  auth,
  supplierController.update_warehouse,
);
router.get(
  "/stores/warehouses/:warehouse_id",
  supplierController.get_warehouse,
);
router.delete(
  "/stores/warehouses/:warehouse_id",
  supplierController.delete_warehouse,
);

// inventory
router.post("/stores/inventory", auth, supplierController.create_inventory);
router.get(
  "/stores/inventory/sku/:sku",
  auth,
  supplierController.get_inventory,
);
router.get(
  "/stores/inventory",
  auth,
  supplierController.get_inventory_by_filter,
); //in-stock, out-of-stock, all,
router.put(
  "/stores/inventory/:sku",
  auth,
  supplierController.update_inventory_stock,
);
router.delete(
  "/stores/inventory/:sku",
  auth,
