...
import { Supplier, ProductImage, Product, ProductVariant, Warehouse, Inventory, ProductReviewLog, supplier_gst_details, supplier_bank_details, SupplierKyc, Settlement, Reseller, Order, LedgerEntry } from "../models/index.js";
        ...
        if (Array.isArray(variants) && variants.length > 0) {
            for (const v of variants) {
                ...
                if (Object.keys(clean).length > 0) {
                    await ProductVariant.update(clean, { where: { variant_id: v.variant_id } });
                }
            }
        }
...
export const updateLiveProductStatus = async (req) => {
    try {
        ...
        if (!status || !["live", "out_of_stock"].includes(status)) {
            return { success: false, message: "Invalid status. Use 'live' or 'out_of_stock'" };
        }
        const product = await Product.findByPk(product_id);
        if (!product) {
    ...
    try {
        ...
        const data = products.map((p) => {
            ...
            return {
                ...
                images: (plain.images || []).map((img) => img.image_url || img).filter(Boolean),
                rejection_reason: plain.rejection_reason ?? "",
                rejection_details: plain.rejection_reason ?? "",
                rejected_at: plain.rejected_at ? new Date(plain.rejected_at).toISOString() : "",
                resubmit_count: 0,
                status: "rejected",
            };
        });
